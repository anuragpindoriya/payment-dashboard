          ...
          <CarouselContent>
            {cards.map((card, index) => (
              <CarouselItem key={index}>
                <div
                  className={`w-[calc(100%-30px)] sm:w-[340px] h-[180px] rounded-[4px] bg-[#0C3F62] text-white px-[16px] pt-[20px] pb-[27px] flex flex-col gap-[8px] ${card?.isAddToGPay ? 'bg-[#4086F4] ' : card.isCardLocked || card?.isCardArchived ? 'bg-[#6A94A5]' : card?.isCardDefault ? 'bg-[#0FA1DB]' : 'bg-[#0C3F62]'} relative`}>
